...
describe("appendChat", () => {
  it("appends a message to the correct AI chat history", () => {
    const game = startPhase(createGame(TEST_PERSONAS), TEST_PHASE_CONFIG);
    const updated = appendChat(game, "red", { role: "player", content: "Hello Ember" });
...
describe("advancePhase", () => {
  ...
  it("marks game complete after phase 3", () => {
    let game = startPhase(createGame(TEST_PERSONAS), TEST_PHASE_CONFIG);
    game = advancePhase(game, { ...TEST_PHASE_CONFIG, phaseNumber: 2, objective: "P2" });
    game = advancePhase(game, { ...TEST_PHASE_CONFIG, phaseNumber: 3, objective: "P3" });
