...
export class RoundCoordinator {
	 ...
	 *      Chat-locked AIs still run their full LLM turn (whispers and tool
	 *      calls dispatched normally); only player-facing chat output is
	 *      flagged so the UI can suppress/gate it.
	...
	private maybeTriggerChatLockout(state: GameState): AiId | null {
		...
		if (this.rng() < CHAT_LOCKOUT_PROBABILITY) {
			// Pick a random non-budget-exhausted AI to lock out
...
function responseToAction(response: AiResponse, aiId: AiId): AiTurnAction {
	...
	// Attach tool call if present (issue #15)
