...
`src/content/goal-pool.ts` no longer exists. The directive role it played has been split: per-game guidance now flows through `SYSADMIN_DIRECTIVE_POOL`, and per-daemon character flows through `PERSONA_GOAL_POOL` and `TEMPERAMENT_POOL` (all in `src/content/pools.ts`). Re-introducing a phase-scoped goal pool would conflict with the single-continuous-game model and would mean re-litigating the restructure decisions above.

The other pools listed in the original ask (temperament, persona-goal, typing-quirk) have been curated organically in the time since — see `TEMPERAMENT_POOL`, `PERSONA_GOAL_POOL` and `TYPING_QUIRK_POOL` in `src/content/pools.ts` (once separate `*-pool.ts` files) and their tests in `src/__tests__/content.test.ts`. No standing work remains there.
