...
- **No save migration required.** Existing Sessions have entities with full flavor and Objective records that reference them; the new code reads them fine (extra flavor fields are ignored). New games are minted with partial flavor under the new flow.
