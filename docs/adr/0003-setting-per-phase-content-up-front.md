...
- `PhaseConfig` shape changes: `objective` becomes a list of `K` objective-pair slots, `initialWorld.items` is replaced by content-pack-driven entity sets, and per-phase rolled counts (`{kRange, nRange, mRange}`) replace fixed counts.
